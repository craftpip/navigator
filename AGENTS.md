...
- Refresh landing-page console screenshots after panel changes with `puppeteer-core` inside the running `navigator` container: open `http://localhost:1994/console` at a `1300x900` viewport and device scale factor `1`; set `localStorage["navigator-theme"]` to `light` or `dark`; reload; wait for `main.app`; pause polling by clicking `.live-toggle`; then capture a viewport-only JPEG at quality `95` to `/app/docs/public/console-light.jpg` or `/app/docs/public/console-dark.jpg`. Rebuild with `docker exec navigator sh -c "cd /app/docs && npm install --include=dev && npm run docs:build"`, confirm both source and `docs-dist/` images are `1300x900` with matching hashes, and restore `docs/` ownership. In a double-quoted shell command, avoid Puppeteer's `page.$(...)` because Bash expands `$()`; use `page.evaluate(() => document.querySelector(...))` instead.
