var CDP_HANDLERS = {
  ...
  'Browser.getWindowForTarget': { type: 'SPECIAL', handler: SpecialHandler.targetGetWindowForTarget },
  'Browser.setWindowBounds': { type: 'SPECIAL', handler: SpecialHandler.targetSetWindowBounds },
  'Browser.getWindowBounds': { type: 'SPECIAL', handler: SpecialHandler.targetGetWindowBounds },
  ...
  // Tab domain is not Chrome CDP — it's navigator-specific (extension only)
...
function routeCDPCommand(message) {
  ...
  // Handlers need both the State module's API (addAttachedTab, isCDPCreatedTab, …)
  // and the live data fields (attachedTabIds, sessionIdToTabId, …). Build a facade:
  var state = State.getState();
  // Session maps live on the canonical raw state so they survive facade rebuilds.
