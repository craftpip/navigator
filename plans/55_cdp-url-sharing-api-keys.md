...
>
> **Pre-existing bug found (not introduced by 55) — FIXED 2026-09-12:** revoking any API key that has `mcp_calls` rows failed with `FOREIGN KEY constraint failed` — `mcp_calls.api_key_id REFERENCES api_keys(id)` (commit 0a71786) has no `ON DELETE` and key ids are written for every MCP call, so the console Revoke button was broken for all used keys. `revokeMcpApiKey()` now NULLs the log references in a transaction before deleting; regression test in `tests/cdp-share.test.js`, verified live (create → use → revoke → 401).
**Created:** 2026-09-11
